...
ruta_excel = os.path.expanduser(
    "https://volkswagengroup.sharepoint.com/sites/VWNA_MONTAJE_TB/Turno%20B%20Mandos/"  # <-- AJUSTAR
)
...
for zona_necesitada in zonas_necesitan:
    ...
    for idx, row in posibles.iterrows():
        nie = row["NIE"]
        if nie not in nies_asignados:
            cambios_directos[nie] = {
                "NIE": nie,
                "Nombre": row["Nombre"],
            ...
            nies_asignados.add(nie)

    asignados_total.extend(asignados)

# Mostrar la tabla de cambios directos sin duplicados

if cambios_directos:
    st.subheader("Tabla de Cambios Directos:")
    cambios_df = pd.DataFrame(cambios_directos.values())
    
    # Ordenar los datos por "Zona Base" (sin afectar la columna de índice)
    cambios_df = cambios_df.sort_values(by="Zona Base")

    # Primero, crear el índice como una nueva columna, comenzando desde 1
    cambios_df["Índice"] = range(1, len(cambios_df) + 1)
    
    # Reorganizar para que el "Índice" esté en la primera columna
    cambios_df = cambios_df[["Índice"] + [col for col in cambios_df.columns if col != "Índice"]]
